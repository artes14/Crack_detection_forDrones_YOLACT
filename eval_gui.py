...
iou_thresholds = [x / 100 for x in range(10, 100, 5)]
coco_cats = {} # Call prep_coco_cats to fill this
coco_cats_inv = {}
color_cache = defaultdict(lambda: {})

def prep_display(dets_out, img, h, w, thres = 30, ignore_masksize = 100, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    """
    ...
    if num_dets_to_consider > 0:
        ...
        for msk in masks.cpu():
            mask = np.reshape(msk, (h, w))  # TORCH.448,448
...
def evalcropimage(net: Yolact, cropsize: int, image, save_path: str = None):
    # crop image
    img_arr, x, y = imgmod.crop_image(cropsize, image=image)
    out_arr, mask_arr = [],[]
    ...
    for idy in range(y):
        for idx in range(x):
            tmp_i[idx, idy] = out_arr[idy*x+idx]
            tmp_m[idx, idy] = mask_arr[idy*x+idx]
    img_tile = concat_tile(tmp_i)
    mask_tile = concat_tile(mask_arr)
...
def evaluate(net:Yolact, image, train_mode=False):
    ...
    if image is not None:
        return evalcropimage(net,448, image)
...
def eval_init(trained_model, config, image=None):
    parse_args()
    set_cfg(config)
    if image is not None:
        if config is None:

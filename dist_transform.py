...
img = Image.open('data/crack_laser_result1/DJI_0030_153_frame.png').convert('L')
...
F_De = cv2.distanceTransform(gray_img, cv2.DIST_L2, 5)
plt.figure(2)
plt.title('F_L2')
plt.imshow(F_De)
plt.show()

F_D4 = cv2.distanceTransform(gray_img, cv2.DIST_C, 5)
plt.figure(3)
plt.title('F_C')
plt.imshow(F_D4)
plt.show()

F_D8 = cv2.distanceTransform(gray_img, cv2.DIST_L2, 5)
...
# !!!! just use opencv distanceTransform !!!!!
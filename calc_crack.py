...
def calc_crackwidth(img_i, img_m):
    ...
    img_origin = img_i
    img_yolact = img_m
    img_yolact = cv2.cvtColor(img_yolact, cv2.COLOR_BGR2GRAY)
    # img_origin = cv2.medianBlur(img_origin,3)
    # img_thresh = cv2.cvtColor(img_origin, cv2.COLOR_BGR2GRAY)
    # img_thresh = cv2.adaptiveThreshold(img_thresh, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 15, 5)
    _, img_thresh = otsu_thres(img_origin)
    # _, img_thresh = cv2.threshold(img_thresh, 0, 255, cv2.THRESH_OTSU)
    ...
    # img_bitand_thin = zhangSuen(img_bitand)
    # cv2.imshow('thin', img_bitand_thin)

    # conv, img_result = convolute_pixel_grd(img_origin, img_bitand, img_bitand_thin)
    conv, img_result = convolute_pixel_DTplusGrad(img_origin, img_bitand, img_bitand_thin)
    ...
    return [mmperpix*min_pix, mmperpix*max_pix, mmperpix*avg_pix], img_result, mask_result
